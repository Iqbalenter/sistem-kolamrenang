...
@section('page-description', 'Manajemen semua booking kolam renang')
    ...
    <div class="border-b border-gray-200">
        ...
        <div class="sm:hidden">
            <select onchange="window.location.href = this.value" class="block w-full px-4 py-3 text-sm font-medium text-gray-700 bg-white border-0 focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                <option value="{{ route('admin.bookings.index') }}" {{ !request('status') && !request('payment') ? 'selected' : '' }}>
                    Semua Booking ({{ $bookings->count() }})
                </option>
                <option value="{{ route('admin.bookings.index', ['status' => 'pending']) }}" {{ request('status') === 'pending' ? 'selected' : '' }}>
                    Pending ({{ $pendingCount }})
                </option>
                <option value="{{ route('admin.bookings.index', ['status' => 'approved']) }}" {{ request('status') === 'approved' ? 'selected' : '' }}>
                    Approved ({{ $approvedCount }})
                </option>
                <option value="{{ route('admin.bookings.index', ['payment' => 'belum_bayar']) }}" {{ request('payment') === 'belum_bayar' ? 'selected' : '' }}>
                    Belum Bayar ({{ $belumBayarCount }})
                </option>
                <option value="{{ route('admin.bookings.index', ['payment' => 'menunggu_konfirmasi']) }}" {{ request('payment') === 'menunggu_konfirmasi' ? 'selected' : '' }}>
        ...
        <nav class="hidden sm:flex sm:space-x-4 lg:space-x-8 px-4 sm:px-6 overflow-x-auto" aria-label="Tabs">
            ...
            <a href="{{ route('admin.bookings.index', ['payment' => 'belum_bayar']) }}" 
               class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-3 sm:py-4 px-1 border-b-2 font-medium text-xs sm:text-sm flex-shrink-0
            ...
            <a href="{{ route('admin.bookings.index', ['payment' => 'menunggu_konfirmasi']) }}" 
               class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-3 sm:py-4 px-1 border-b-2 font-medium text-xs sm:text-sm flex-shrink-0
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kolam & Waktu</th>
                ...
                <div class="border-b border-gray-200 p-4 hover:bg-gray-50">
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center">
                            ...
                            <div>
                                <h4 class="text-sm font-medium text-gray-900">{{ $booking->nama_pemesan }}</h4>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-3 text-xs">
                        <div>
                            <p class="text-gray-500">Kolam:</p>
                            <p class="font-medium text-gray-900">{{ $booking->jenis_kolam_label ?? 'Kolam Utama' }}</p>
                        </div>
                        <div>
                            <p class="text-gray-500">Tanggal:</p>
                            <p class="font-medium text-gray-900">{{ $booking->tanggal_booking->format('d M Y') }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="font-medium text-gray-900">Rp {{ number_format($booking->total_harga, 0, ',', '.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        @if($booking->status === 'pending')
                            <button onclick="approveBooking({{ $booking->id }})"
                                class="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-700 rounded-md hover:bg-green-200 transition-colors text-xs">
                                <i class="fas fa-check mr-1"></i>
                                Approve
                            </button>
                            <button onclick="showRejectModal({{ $booking->id }})"
...
@else
    <div class="bg-white rounded-xl shadow-lg p-8 sm:p-12 text-center">
        <i class="fas fa-calendar-times text-4xl sm:text-6xl text-gray-300 mb-3 sm:mb-4"></i>
        <h3 class="text-lg sm:text-xl font-semibold text-gray-900 mb-2">Tidak ada booking ditemukan</h3>
        <p class="text-sm sm:text-base text-gray-500">Belum ada booking yang perlu dikelola saat ini.</p>
    </div>
@endif

<!-- Modal Reject Booking -->
<div id="rejectModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-10 sm:top-20 mx-auto p-4 sm:p-5 border w-full max-w-md sm:max-w-lg shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            <h3 class="text-base sm:text-lg font-medium text-gray-900">Tolak Booking</h3>
            <form id="rejectForm" method="POST">
                @csrf
                @method('PATCH')
                <div class="mt-4">
                    <textarea name="catatan_admin" placeholder="Alasan penolakan..." class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm sm:text-base" rows="3" required></textarea>
                </div>
                <div class="items-center px-2 sm:px-4 py-3">
                    <button type="button" onclick="closeRejectModal()" class="px-3 sm:px-4 py-2 bg-gray-500 text-white text-sm font-medium rounded-md shadow-sm hover:bg-gray-600 mr-2">
                        Batal
                    </button>
                    <button type="submit" class="px-3 sm:px-4 py-2 bg-red-500 text-white text-sm font-medium rounded-md shadow-sm hover:bg-red-600">
                        Tolak
                    </button>
                </div>
            </form>
...
<div id="rejectPaymentModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-10 sm:top-20 mx-auto p-4 sm:p-5 border w-full max-w-md sm:max-w-lg shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            <h3 class="text-base sm:text-lg font-medium text-gray-900">Tolak Pembayaran</h3>
            <p class="text-xs sm:text-sm text-gray-600 mt-2">Bukti pembayaran akan dihapus dan user harus upload ulang</p>
            <form id="rejectPaymentForm" method="POST">
                @csrf
                @method('PATCH')
                <div class="mt-4">
                    <textarea name="catatan_admin" placeholder="Alasan penolakan pembayaran..." class="w-full px-3 py-2 border border-gray-300 rounded-md text-sm sm:text-base" rows="3" required></textarea>
                </div>
                <div class="items-center px-2 sm:px-4 py-3">
                    <button type="button" onclick="closeRejectPaymentModal()" class="px-3 sm:px-4 py-2 bg-gray-500 text-white text-sm font-medium rounded-md shadow-sm hover:bg-gray-600 mr-2">
                        Batal
                    </button>
                    <button type="submit" class="px-3 sm:px-4 py-2 bg-red-500 text-white text-sm font-medium rounded-md shadow-sm hover:bg-red-600">
                        Tolak Pembayaran
...
function showRejectPaymentModal(bookingId) {
    document.getElementById('rejectPaymentForm').action = `/admin/bookings/${bookingId}/reject-payment`;
...
function confirmPayment(bookingId) {
    if (confirm('Konfirmasi pembayaran untuk booking ini?')) {
        fetch(`/admin/bookings/${bookingId}/confirm-payment`, {
            method: 'PATCH',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Content-Type': 'application/json',
